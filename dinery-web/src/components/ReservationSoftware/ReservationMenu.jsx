      ...
      <div className="space-y-1.5">
        {LANGUAGES.map(lang => (
          <div key={lang.code} className={`flex items-start gap-2 ${activeLang !== 'all' && activeLang !== lang.code ? 'hidden' : ''}`}>
            <span className="flex-shrink-0 mt-1 text-xs font-bold text-gray-400 w-6 text-center">{lang.flag}</span>
            {multiline ? (
              <textarea
                ...
                className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#fe8a24] resize-none transition-colors"
              />
            ) : (
              <input
                type="text"
                ...
                onChange={e => onChange({ ...value, [lang.code]: e.target.value })}
                placeholder={`${label || 'Text'} in ${lang.name}…`}
                className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#fe8a24] transition-colors"
              />
            )}
          </div>
        ))}
...
const ItemModal = ({ item, categories, onSave, onClose, saving }) => {
  ...
  const [tab, setTab]           = useState('basic'); // basic | allergens | attributes
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Fill in details for each language</p>
          </div>
        ...
        {/* Lang tabs */}
        <div className="flex items-center gap-1 px-6 py-2 bg-gray-50 border-b border-gray-100 flex-shrink-0">
          <span className="text-xs text-gray-400 mr-2">Language:</span>
          <button onClick={() => setActiveLang('all')}
            className={`text-xs px-2.5 py-1 rounded-lg font-semibold transition-all ${activeLang === 'all' ? 'bg-[#fe8a24] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-[#fe8a24]'}`}>
            All
          </button>
          {LANGUAGES.map(l => (
            <button key={l.code} onClick={() => setActiveLang(l.code)}
              className={`text-xs px-2.5 py-1 rounded-lg font-semibold transition-all ${activeLang === l.code ? 'bg-[#fe8a24] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-[#fe8a24]'}`}>
              {l.flag} {l.label}
            </button>
          ))}
        </div>
        ...
        {/* Tabs */}
        <div className="flex items-center gap-0 border-b border-gray-100 flex-shrink-0 px-6">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          {tab === 'basic' && (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={form.subcategory} onChange={e => set('subcategory', e.target.value)}
                    ...
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#fe8a24] disabled:bg-gray-50 disabled:text-gray-400">
            ...
            {/* Guest capacity (for public page filtering) */}
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">👥 Min Guests to Show</label>
                  <input type="number" min="0" max="999" value={form.minCapacity || ''}
                    ...
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-base focus:outline-none focus:border-[#fe8a24] focus:ring-2 focus:ring-[#fe8a24]/20 transition-all" />
                  <p className="text-xs text-gray-400 mt-1">Show item when party is at least this size</p>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">👥 Max Guests to Show</label>
                  <input type="number" min="0" max="999" value={form.maxCapacity || ''}
                    onChange={e => set('maxCapacity', parseInt(e.target.value) || 0)}
                    placeholder="0 = always show"
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-base focus:outline-none focus:border-[#fe8a24] focus:ring-2 focus:ring-[#fe8a24]/20 transition-all" />
                  <p className="text-xs text-gray-400 mt-1">Show item when party is at most this size</p>
                </div>
              </div>
          ...
          {tab === 'allergens' && (
            <div>
              <p className="text-sm text-gray-500 mb-4">Select all allergens present in this item.</p>
                ...
                {ALLERGENS.map(a => {
                  ...
                  return (
                    <button key={a.id} onClick={() => toggleArr('allergens', a.id)}
                      ...
                      <span>{a.label}</span>
                      {active && <FiCheck className="w-3.5 h-3.5 ml-auto text-amber-600" />}
                    </button>
                  );
                })}
              </div>
              {form.allergens.length > 0 && (
                <div className="mt-4 p-3 bg-amber-50 rounded-xl border border-amber-200">
                  <p className="text-xs font-semibold text-amber-700 mb-2">Selected allergens:</p>
          ...
          {tab === 'attributes' && (
            <div>
              <p className="text-sm text-gray-500 mb-4">Tag this item with dietary and service attributes.</p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {ATTRIBUTES.map(a => {
                  ...
                  return (
                    <button key={a.id} onClick={() => toggleArr('attributes', a.id)}
                      ...
                      <span className="text-base">{a.icon}</span>
                      <span>{a.label}</span>
                      {active && <FiCheck className="w-3.5 h-3.5 ml-auto" style={{ color: a.color }} />}
                    </button>
                  );
                })}
              </div>
              {form.attributes.length > 0 && (
                <div className="mt-4 p-3 bg-gray-50 rounded-xl border border-gray-200">
                  <p className="text-xs font-semibold text-gray-600 mb-2">Selected attributes:</p>
                  <div className="flex flex-wrap gap-1.5">
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
            <button onClick={onClose} className="px-5 py-2 border border-gray-200 text-gray-600 rounded-lg text-sm hover:bg-gray-50">
              Cancel
            </button>
            <button onClick={() => onSave(form)} disabled={saving}
              className="flex items-center gap-2 px-6 py-2 bg-[#fe8a24] hover:bg-[#ff9d47] text-white rounded-lg text-sm font-semibold disabled:opacity-50 transition-colors">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[85vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <h3 className="text-lg font-bold text-gray-900">{category ? 'Edit Category' : 'Add Category'}</h3>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full text-gray-500"><FiX className="w-5 h-5" /></button>
        </div>
          ...
          {['all', ...LANGUAGES.map(l => l.code)].map(code => {
            ...
            return (
              <button key={code} onClick={() => setActiveLang(code)}
                className={`text-xs px-2.5 py-1 rounded-lg font-semibold transition-all ${activeLang === code ? 'bg-[#fe8a24] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-[#fe8a24]'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {COLORS.map(col => (
                <button key={col} onClick={() => set('color', col)}
                  className={`w-8 h-8 rounded-lg transition-all ${form.color === col ? 'ring-2 ring-offset-2 ring-gray-400 scale-110' : 'hover:scale-105'}`}
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm text-gray-700">{form.active ? 'Category active' : 'Category hidden'}</span>
          </label>

          {/* Subcategories */}
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-2">Subcategories</label>
            <div className="space-y-1.5 mb-2">
              {(form.subcategories || []).map(sub => (
                <div key={sub.id} className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
                  <span className="flex-1 text-sm text-gray-700">{sub.name?.en || sub.id}</span>
                  <button onClick={() => removeSubcategory(sub.id)} className="text-gray-400 hover:text-red-500 transition-colors">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={addSubcategory} disabled={!newSubName.trim()}
                className="px-3 py-2 bg-[#fe8a24] text-white rounded-lg text-sm font-semibold disabled:opacity-40 hover:bg-[#ff9d47] transition-colors">
                Add
              </button>
            </div>
          </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-end gap-3 flex-shrink-0">
          <button onClick={onClose} className="px-5 py-2 border border-gray-200 text-gray-600 rounded-lg text-sm hover:bg-gray-50">Cancel</button>
          <button onClick={() => onSave(form)} disabled={saving}
            className="flex items-center gap-2 px-6 py-2 bg-[#fe8a24] hover:bg-[#ff9d47] text-white rounded-lg text-sm font-semibold disabled:opacity-50 transition-colors">
            {saving ? <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <FiCheck className="w-4 h-4" />}
            {saving ? 'Saving…' : 'Save'}
...
export default function ReservationMenu({ selectedRestaurant, onClose }) {
  ...
  // UI state
  const [selectedCat,    setSelectedCat]    = useState(null); // category id filter
  ...
  const [deleteConf,  setDeleteConf]  = useState(null); // { type, id, name }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 flex-shrink-0"
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#fe8a24] flex items-center justify-center flex-shrink-0">
              <span className="text-white text-xl">🍽️</span>
            </div>
            <div>
              <h2 className="text-lg font-bold text-white">Menu Management</h2>
              <p className="text-xs text-slate-400">{selectedRestaurant?.name}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1 bg-white/10 rounded-lg p-1">
              {LANGUAGES.map(l => (
                <button key={l.code} onClick={() => setViewLang(l.code)}
                  className={`text-xs px-2.5 py-1 rounded-md font-semibold transition-all ${viewLang === l.code ? 'bg-[#fe8a24] text-white' : 'text-slate-300 hover:text-white'}`}>
                  {l.flag} {l.label}
                </button>
              ))}
            </div>
        ...
        <div className="flex flex-1 overflow-hidden min-h-0">
          ...
          <div className="w-60 flex-shrink-0 border-r border-gray-200 flex flex-col bg-gray-50">
            <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
              ...
              <button onClick={() => setCatModal('add')}
                className="w-6 h-6 rounded-lg bg-[#fe8a24] text-white flex items-center justify-center hover:bg-[#ff9d47] transition-colors" title="Add category">
            ...
            <div className="flex-1 overflow-y-auto py-2">
              ...
              <button
                ...
                <span>All Items</span>
                <span className="text-xs bg-gray-200 text-gray-600 px-1.5 py-0.5 rounded-full">{menuItems.length}</span>
              </button>
                  ...
                  <div key={cat.id}>
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {!hasSubs && <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: cat.color || '#fe8a24' }} />}
                        <span className="text-sm font-semibold truncate">{cat.name?.[viewLang] || cat.name?.en || 'Unnamed'}</span>
                        {!cat.active && <FiEyeOff className="w-3 h-3 text-gray-400 flex-shrink-0" />}
                      </div>
                    ...
                    {isExpanded && hasSubs && (cat.subcategories || []).map(sub => (
                      <button key={sub.id}
                        ...
                        className={`w-full text-left pl-10 pr-4 py-2 text-sm transition-colors flex items-center justify-between ${
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="px-5 py-3 border-b border-gray-200 flex items-center gap-3 flex-shrink-0 bg-white">
              {/* Breadcrumb */}
              <div className="flex items-center gap-1 text-sm font-semibold text-gray-700 min-w-0">
                {activeCat ? (
                  <>
                    <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: activeCat.color || '#fe8a24' }} />
                ...
                <span className="ml-2 text-xs font-normal text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full flex-shrink-0">
              ...
              <div className="ml-auto flex items-center gap-2">
                ...
                <div className="flex items-center gap-1.5 bg-gray-100 rounded-lg px-3 py-1.5">
                  ...
                  <input value={searchQ} onChange={e => setSearchQ(e.target.value)}
                    ...
                    className="bg-transparent text-sm text-gray-700 focus:outline-none w-36 placeholder-gray-400" />
                  {searchQ && <button onClick={() => setSearchQ('')} className="text-gray-400 hover:text-gray-600"><FiX className="w-3 h-3" /></button>}
                </div>

                {/* Layout toggle */}
                <div className="flex rounded-lg border border-gray-200 overflow-hidden">
                  <button onClick={() => setViewLayout('list')}
                    className={`p-1.5 ${viewLayout === 'list' ? 'bg-[#fe8a24] text-white' : 'text-gray-500 hover:bg-gray-50'}`}>
                    <FiList className="w-4 h-4" />
                  </button>
                  <button onClick={() => setViewLayout('grid')}
                    className={`p-1.5 ${viewLayout === 'grid' ? 'bg-[#fe8a24] text-white' : 'text-gray-500 hover:bg-gray-50'}`}>
                ...
                <button onClick={() => setItemModal('add')}
                  className="flex items-center gap-2 bg-[#fe8a24] hover:bg-[#ff9d47] text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {filteredItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  <div className="w-16 h-16 rounded-2xl bg-gray-100 flex items-center justify-center mb-4 text-3xl">🍽️</div>
                  ...
                  <button onClick={() => setItemModal('add')}
                    className="mt-4 flex items-center gap-2 bg-[#fe8a24] text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-[#ff9d47] transition-colors">
                ...
                <div className="space-y-1.5">
                  {/* Header row */}
                  <div className="grid px-4 py-2 text-xs font-bold text-gray-400 uppercase tracking-wider"
                  ...
                  {filteredItems.map(item => {
                    ...
                    return (
                      <div key={item.id}
                        className={`grid items-center px-4 py-3 rounded-xl border transition-all hover:shadow-sm cursor-pointer group ${
                      ...
                      >
                        ...
                        <div className="min-w-0 pr-3">
                          <div className="flex items-center gap-2">
                            {cat && <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: cat.color || '#fe8a24' }} />}
                            <span className="text-sm font-semibold text-gray-900 truncate">{nameStr}</span>
                            {!item.active && <FiEyeOff className="w-3 h-3 text-gray-400 flex-shrink-0" />}
                          </div>
                          {descStr && <p className="text-sm text-gray-500 truncate mt-1 ml-3">{descStr}</p>}
                          {(item.minCapacity > 0 || item.maxCapacity > 0) && (
                            <p className="text-xs text-blue-500 mt-0.5 ml-3">
                              👥 {item.minCapacity > 0 && item.maxCapacity > 0
                        ...
                        <div className="flex flex-wrap gap-1">
                          ...
                          {(item.allergens || []).length > 4 && (
                            <span className="text-[10px] text-gray-400">+{item.allergens.length - 4}</span>
                          )}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          ...
                          {(item.attributes || []).length > 3 && (
                            <span className="text-[10px] text-gray-400">+{item.attributes.length - 3}</span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                          <button onClick={() => toggleItemActive(item)} title={item.active ? 'Hide' : 'Show'}
                            ...
                            {item.active ? <FiEye className="w-3.5 h-3.5" /> : <FiEyeOff className="w-3.5 h-3.5" />}
                          </button>
                          <button onClick={() => setItemModal(item)}
                            className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-[#fe8a24] transition-colors">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
                  {filteredItems.map(item => {
                    ...
                    return (
                      ...
                      >
                        {/* Color bar */}
                        <div className="h-1.5" style={{ background: cat?.color || '#fe8a24' }} />
                        <div className="p-4">
                          <div className="flex items-start justify-between gap-2 mb-1">
                            <p className="text-sm font-bold text-gray-900 leading-tight">{nameStr}</p>
                            <span className="text-sm font-bold text-[#fe8a24] flex-shrink-0">
                              {item.price ? `${item.price},-` : ''}
                            </span>
                          </div>
                          {descStr && <p className="text-xs text-gray-400 line-clamp-2 mb-2">{descStr}</p>}
                          <div className="flex flex-wrap gap-1 mt-auto">
                            {(item.attributes || []).map(id => <AttributeBadge key={id} id={id} small />)}
                            {(item.allergens || []).slice(0, 3).map(id => <AllergenBadge key={id} id={id} small />)}
                          </div>
                          <div className="flex items-center gap-1 mt-2 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                            <button onClick={() => setItemModal(item)}
                              className="flex-1 text-xs py-1 bg-gray-100 hover:bg-[#fe8a24] hover:text-white text-gray-600 rounded-lg transition-colors font-medium">
                              Edit
                            </button>
                            <button onClick={() => setDeleteConf({ type: 'item', id: item.id, name: nameStr })}
            ...
            {/* Footer stats */}
            <div className="px-5 py-2 border-t border-gray-100 flex items-center gap-4 bg-gray-50 flex-shrink-0">
              <span className="text-xs text-gray-400">
                <span className="font-semibold text-gray-600">{menuItems.length}</span> total items ·{' '}
      ...
      {/* ── Toast ── */}
      {toast && (
        <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-3 px-5 py-3 rounded-2xl shadow-2xl text-sm font-semibold ${
          toast.type === 'error' ? 'bg-red-500 text-white' : 'bg-gray-900 text-white'
        }`}>
          {toast.type === 'error' ? <FiAlertCircle className="w-4 h-4" /> : <FiCheck className="w-4 h-4 text-green-400" />}
          {toast.msg}
        </div>
      )}
      ...
      {deleteConf && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4">
            ...
            <h3 className="text-lg font-bold text-gray-900 text-center mb-1">Delete {deleteConf.type}?</h3>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmDelete} disabled={saving}
                className="flex-1 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-xl text-sm font-semibold disabled:opacity-50 transition-colors">
